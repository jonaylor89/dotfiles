  ...
  engine_config:
    ...
    suboptions:
        ...
        suboptions:
          ...
          autovacuum_max_workers:
            ...
            - Specifies the maximum number of autovacuum processes (other than the
              autovacuum launcher) that may be running at any one time.
            - The default is three.
            - This parameter can only be set at server start.
            required: false
            type: int
          ...
          bgwriter_flush_after:
            description:
            - Whenever more than bgwriter_flush_after bytes have been written by the
              background writer, attempt to force the OS to issue these writes to
          ...
          bgwriter_lru_multiplier:
            description:
            - The average recent need for new buffers is multiplied by bgwriter_lru_multiplier
              to arrive at an estimate of the number that will be needed during the
              next round, (up to bgwriter_lru_maxpages).
            - "1.0 represents a \u201Cjust in time\u201D policy of writing exactly\
              \ the number of buffers predicted to be needed."
            - Larger values provide some cushion against spikes in demand, while smaller
              values intentionally leave writes to be done by server processes.
            - The default is 2.0.
            required: false
            type: float
          deadlock_timeout:
          ...
          pg_stat_statements.track:
            ...
            description:
            - Controls which statements are counted.
            - Specify 'top' to track top-level statements (those issued directly by
              clients), 'all' to also track nested statements (such as statements
              invoked within functions), or 'none' to disable statement statistics
              collection.
            - The default value is 'top'.
            required: false
            type: str
          ...
          track_io_timing:
            description:
            - Enables timing of database I/O calls.
            - This parameter is off by default, because it will repeatedly query the
              operating system for the current time, which may cause significant overhead
              on some platforms.
            required: false
            type: str
          wal_sender_timeout:
          ...
          wal_writer_delay:
            description:
            - WAL flush interval in milliseconds.
            - Note that setting this value to lower than the default 200ms may negatively
              impact performance.
            required: false
            type: int
        type: dict
      pg_stat_monitor_enable:
        ...
        - Enable the pg_stat_monitor extension.
        - Enabling this extension will cause the cluster to be restarted.
        - When this extension is enabled, pg_stat_statements results for utility commands
          are unreliable.
        required: false
        type: bool
      pglookout:
        ...
        suboptions:
          max_failover_replication_time_lag:
            description:
            - Number of seconds of master unavailability before triggering database
              failover to standby.
            required: false
            type: int
        type: dict
      ...
      work_mem:
        ...
        - Sets the maximum amount of memory to be used by a query operation (such
          as a sort or hash table) before writing to temporary disk files, in MB.
        - Default is 1MB + 0.075% of total RAM (up to 32MB).
        required: false
        type: int
    type: dict
...
requirements:
- python >= 3
short_description: Create, read, and update a Linode PostgreSQL database.
"""
EXAMPLES = r"""
...
- name: Create a PostgreSQL database with three nodes
  linode.cloud.database_postgresql_v2:
    ...
    - 0.0.0.0/0
    state: present
- name: Create a PostgreSQL database with an explicit maintenance schedule and engine
    configuration
  linode.cloud.database_postgresql_v2:
    label: my-db
    region: us-mia
