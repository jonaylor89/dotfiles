#!/usr/bin/python
# -*- coding: utf-8 -*-

"""This module allows users to list Linode VLANs."""

from __future__ import absolute_import, division, print_function

import ansible_collections.linode.cloud.plugins.module_utils.doc_fragments.vlan_list as docs
from ansible_collections.linode.cloud.plugins.module_utils.linode_common_list import (
    ListModule,
)

module = ListModule(
    result_display_name="VLANs",
    result_field_name="vlans",
    endpoint_template="/networking/vlans",
    result_docs_url="https://techdocs.akamai.com/linode-api/reference/get-vlans",
    examples=docs.specdoc_examples,
    result_samples=docs.result_vlan_samples,
)

SPECDOC_META = module.spec

DOCUMENTATION = r"""
author:
- Luke Murphy (@decentral1se)
- Charles Kenney (@charliekenney23)
- Phillip Campbell (@phillc)
- Lena Garber (@lbgarber)
- Jacob Riddle (@jriddle)
- Zhiwei Liang (@zliang)
- Ye Chen (@yechen)
- Youjung Kim (@ykim)
- Vinay Shanthegowda (@vshanthe)
- Erik Zilber (@ezilber)
description:
- List and filter on VLANs.
module: vlan_list
notes: []
options:
  count:
    description:
    - The number of VLANs to return.
    - If undefined, all results will be returned.
    required: false
    type: int
  filters:
    description:
    - A list of filters to apply to the resulting VLANs.
    elements: dict
    required: false
    suboptions:
      name:
        description:
        - The name of the field to filter on.
        - Valid filterable fields can be found [here](https://techdocs.akamai.com/linode-api/reference/get-vlans).
        required: true
        type: str
      values:
        description:
        - A list of values to allow for this field.
        - Fields will pass this filter if at least one of these values matches.
        elements: str
        required: true
        type: list
    type: list
  order:
    choices:
    - desc
    - asc
    default: asc
    description:
    - The order to list VLANs in.
    required: false
    type: str
  order_by:
    description:
    - The attribute to order VLANs by.
    required: false
    type: str
requirements:
- python >= 3
short_description: List and filter on VLANs.
"""
EXAMPLES = r"""
- name: List all of the VLANs for the current Linode Account
  linode.cloud.vlan_list:
    api_version: v4beta
- name: List all VLANs in the us-southeast region
  linode.cloud.vlan_list:
    api_version: v4beta
    filters:
    - name: region
      values: us-southeast
"""
RETURN = r"""
vlans:
  description: The returned VLANs.
  elements: dict
  returned: always
  sample:
  - - created: '2020-01-01T00:01:01'
      label: vlan-example
      linodes:
      - 111
      - 222
      region: ap-west
  type: list
"""

if __name__ == "__main__":
    module.run()
