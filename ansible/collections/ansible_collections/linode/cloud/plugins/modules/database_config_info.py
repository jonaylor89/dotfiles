...
options:
  engine:
    description:
    - The Database Engine of the Configuration to resolve.
    required: true
    type: str
requirements:
- python >= 3
short_description: Get info about a Linode Configuration.
"""
EXAMPLES = r"""
  ...
  - binlog_retention_period:
    ...
    mysql:
      connect_timeout:
        ...
        type: integer
      default_time_zone:
        description: Default server time zone as an offset from UTC (from -12:00 to
          +12:00), a time zone name, or 'SYSTEM' to use the MySQL server default.
        example: +03:00
        maxLength: 100
        minLength: 2
        pattern: ^([-+][\d:]*|[\w/]*)$
      ...
      innodb_change_buffer_max_size:
        description: Maximum size for the InnoDB change buffer, as a percentage of
          the total size of the buffer pool. Default is 25
        example: 30
        maximum: 50
        minimum: 0
        requires_restart: false
      ...
      innodb_flush_neighbors:
        description: 'Specifies whether flushing a page from the InnoDB buffer pool
          ...
          pages in the same extent are not flushed, 1 - flush contiguous dirty pages
          in the same extent, 2 - flush dirty pages in the same extent'
        example: 0
        maximum: 2
        minimum: 0
        requires_restart: false
      ...
      innodb_rollback_on_timeout:
        description: When enabled a transaction timeout causes InnoDB to abort and
          roll back the entire transaction. Changing this parameter will lead to a
      ...
      max_heap_table_size:
        ...
        type: integer
      net_buffer_length:
        description: Start sizes of connection buffer and result buffer. Default is
          16384 (16K). Changing this parameter will lead to a restart of the MySQL
          service.
        example: 16384
        maximum: 1048576
        minimum: 1024
      ...
      net_read_timeout:
        ...
        type: integer
      net_write_timeout:
        description: The number of seconds to wait for a block to be written to a
      ...
      sort_buffer_size:
        ...
        type: integer
      sql_mode:
        description: Global SQL mode. Set to empty to use MySQL server defaults. When
          creating a new service and not setting this field Akamai default SQL mode
      ...
      sql_require_primary_key:
        description: Require primary key to be defined for new tables or old tables
          ...
          have primary keys because various functionality may break if any large table
          is missing them.
        example: true
        requires_restart: false
        type: boolean
